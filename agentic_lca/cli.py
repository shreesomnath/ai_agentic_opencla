...
def write_lca_report_file(product_name, loaded_bom_path, exchanges_list, active_report, simulation_mode, port=8080, llm_agent=None):
    if not exchanges_list:
        print("No product model or BOM loaded. Report generation skipped.")
        return None
        
    import datetime
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    safe_name = "".join([c if c.isalnum() else "_" for c in product_name]).lower()
    filename = f"{safe_name}_lca_report.md"
...
The product model was synthesized using the feedstock recipe loaded from `{loaded_bom_path or 'Dynamic Prompt Ingestion'}`.
...
"""
    for ex in exchanges_list:
        if isinstance(ex, dict):
            ...
            name = ex.flow.name if (ex.flow and hasattr(ex.flow, 'name')) else "Unknown"
            amount = ex.amount
            unit = ex.unit.name if (ex.unit and hasattr(ex.unit, 'name')) else "kg"
        md += f"| {name} | {amount:.4f} | {unit} |\n"
...
| :--- | :--- | :--- | :--- | :--- | :--- |
"""
        metrics = active_report.get("metrics", {})
        for key, details in metrics.items():
            baseline_val = details.get("baseline", 0.0)
            opt_val = details.get("optimized", 0.0)
            diff = details.get("difference", 0.0)
            pct_change = details.get("percentage_change", 0.0)
            unit = details.get("unit", "")
            
            md += f"| {key} | {baseline_val:.6f} | {opt_val:.6f} | {diff:+.6f} | {pct_change:+.2f}% | {unit} |\n"
            
        md += f"""
### Trade-off Interpretation
- **Substituted From:** `{active_report.get('substituted_from')}`
- **Substituted To:** `{active_report.get('substituted_to')}`

"""
        gwp_pct = metrics.get("Global Warming", {}).get("percentage_change", 0.0)
        cost_pct = metrics.get("Feedstock Cost", {}).get("percentage_change", 0.0)
        water_pct = metrics.get("Water Consumption", {}).get("percentage_change", 0.0)
        acid_pct = metrics.get("Acidification", {}).get("percentage_change", 0.0)
        
        is_pareto = gwp_pct <= 0 and cost_pct <= 0 and water_pct <= 0 and acid_pct <= 0
        if is_pareto:
            md += "The feedstock substitution represents a **strictly Pareto-improving state**, decoupling environmental footprint from operational economic expenditures without burden-shifting.\n"
        else:
            md += "The configuration shows trade-offs between environmental savings and procurement costs, requiring multi-criteria decision prioritization.\n"
            
        if llm_agent:
            print("Generating explanation text for the report...")
            try:
                justification = llm_agent.generate_engineering_justification(active_report)
                md += f"""
### 3. Engineering Justification Report (LLM)
{justification}
"""
            except Exception as e_just:
                md += f"\nFailed to retrieve LLM justification: {e_just}\n"
    else:
...
"""
    ...
    try:
        with open(filename, "w") as f:
            f.write(md)
        print(f"🟢 Complete LCA study report saved successfully to '{filename}'")
    except Exception as e:
    ...
    try:
        with open(brain_path, "w") as f:
            f.write(md)
        print(f"🟢 Copied report artifact to chat console: '{brain_path}'")
    except Exception:
        pass
...
def run_system_setup_diagnostics():
    print("="*80)
    print("              AGENTIC-LCA SYSTEM SETUP & DIAGNOSTICS")
    print("="*80)
...
def run_interactive_cli_chat(port):
    ...
    # State variables
    product_name = "Next-Gen Silicon Solar Cell Module"
    ...
    def do_load_bom(bom_path):
        nonlocal loaded_bom_path, exchanges_list, raw_exchanges, temp_flow_id, temp_proc_id, temp_sys_id, method_desc, simulation_mode, product_name
        
        if bom_path != "temp_product_bom.csv":
            if "sample_bom" in bom_path:
                product_name = "Next-Gen Silicon Solar Cell Module"
            else:
                product_name = f"Imported BOM ({os.path.basename(bom_path)})"
...
"""
    ...
    def save_lca_report():
        nonlocal product_name, loaded_bom_path, exchanges_list, active_report, simulation_mode, port
        write_lca_report_file(
            product_name=product_name,
            loaded_bom_path=loaded_bom_path,
    ...
    while True:
        try:
            ...
            if query_lower in ["exit", "quit", "end"]:
                print("Ending interactive session. Saving report and cleaning up database resources...")
                save_lca_report()
                do_db_cleanup()
                print("Goodbye!")
                break
                
            if query_lower in ["save", "export"]:
                save_lca_report()
                continue
...
def main():
    ...
    if "--test-connection" in sys.argv or "-t" in sys.argv:
        ...
        try:
            ...
            if active:
                ...
                print(f"You can connect using: agentic-lca --port {active[0]}")
            else:
    ...
    try:
        ...
        print("\n[10/10] Saving complete LCA study report...")
        try:
            exchanges_list = []
            for ex in exchanges:
                exchanges_list.append({
                    "id": ex.flow.id if ex.flow else "",
                    "name": ex.flow.name if ex.flow else "",
                    "amount": ex.amount,
                    "unit": ex.unit.name if ex.unit else "kg"
                })
            write_lca_report_file(
                product_name=module_flow.name,
                loaded_bom_path=bom_path,
                exchanges_list=exchanges_list,
                active_report=report,
                simulation_mode=simulation_mode,
            ...
            print(f"[Warning] Failed to save study report: {e_report}")
        print("="*80)
